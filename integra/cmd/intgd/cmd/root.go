...
// NewRootCmd creates a new root command for intgd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	// we "pre"-instantiate the application for getting the injected/configured encoding configuration
	// and the CLI options for the modules
	// add keyring to autocli opts
	tempApp := integra.NewIntegraApp(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
	...
	initClientCtx := client.Context{}.
		...
		WithHomeDir(integra.MustGetDefaultNodeHome()).
		WithViper("").
	...
	rootCmd := &cobra.Command{
		Use:   "intgd",
		Short: "Integra Layer blockchain node",
...
func initRootCmd(rootCmd *cobra.Command, app *integra.IntegraApp) {
	cfg := sdk.GetConfig()
	cfg.Seal()

	defaultNodeHome := integra.MustGetDefaultNodeHome()
...
func appExport(
	...
	if height != -1 {
		app = integra.NewIntegraApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = integra.NewIntegraApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID))
	}
