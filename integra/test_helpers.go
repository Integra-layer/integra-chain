...
func setup(withGenesis bool, invCheckPeriod uint, chainID string, _ uint64) (*IntegraApp, GenesisState) {
	...
	app := NewIntegraApp(log.NewNopLogger(), db, nil, true, appOptions, baseapp.SetChainID(chainID))
	if withGenesis {
...
// Setup initializes a new IntegraApp. A Nop logger is set in IntegraApp.
func Setup(t *testing.T, chainID string, evmChainID uint64) *IntegraApp {
	t.Helper()
	...
	balance := banktypes.Balance{
		...
		Coins:   sdk.NewCoins(sdk.NewCoin(IntegraChainDenom, math.NewInt(100000000000000))),
	}
...
// SetupWithGenesisValSet initializes a new IntegraApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the simapp from first genesis
// account. A Nop logger is set in IntegraApp.
func SetupWithGenesisValSet(t *testing.T, chainID string, evmChainID uint64, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *IntegraApp {
	t.Helper()
