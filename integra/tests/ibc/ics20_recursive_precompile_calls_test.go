...
func (suite *ICS20RecursivePrecompileCallsTestSuite) prepareStakingRewards(ctx sdk.Context, stkRs ...stakingRewards) (sdk.Context, error) {
	for _, r := range stkRs {
		...
		bondDenom, err := suite.chainA.App.(*integra.IntegraApp).StakingKeeper.BondDenom(suite.chainA.GetContext())
		...
		if err := suite.chainA.App.(*integra.IntegraApp).GetDistrKeeper().AllocateTokensToValidator(ctx, r.Validator, allocatedRewards); err != nil {
			return ctx, err
		}
	}
	return ctx, nil
...
func (suite *ICS20RecursivePrecompileCallsTestSuite) mintCoinsForDistrMod(ctx sdk.Context, amount sdk.Coins) error {
	...
	if err := suite.chainA.App.(*integra.IntegraApp).GetBankKeeper().MintCoins(
		ctx,
	...
	return suite.chainA.App.(*integra.IntegraApp).GetBankKeeper().SendCoinsFromModuleToModule(
		ctx,
		minttypes.ModuleName,
...
func (suite *ICS20RecursivePrecompileCallsTestSuite) setupContractForTesting(
	...
	evmAppA := suite.chainA.App.(*integra.IntegraApp)
...
func (suite *ICS20RecursivePrecompileCallsTestSuite) SetupTest() {
	...
	evmAppB := suite.chainB.App.(*integra.IntegraApp)
