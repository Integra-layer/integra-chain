...
function extractChainIDFromGo(goFilePath) {
  try {
    // First try Integra-specific config
    const integraConfigPath = path.join(
      __dirname,
      "../../integra/integra_config.go",
    );
    if (fs.existsSync(integraConfigPath)) {
      const integraContent = fs.readFileSync(integraConfigPath, "utf8");
      const integraMatch = integraContent.match(
        /IntegraEVMChainID\s*(?:uint64\s*)?=\s*(\d+)/,
      );
      if (integraMatch) {
        const chainId = parseInt(integraMatch[1], 10);
        logger.info(
          `Extracted IntegraEVMChainID from integra config: ${chainId}`,
        );
        return chainId;
      }
    }
  ...
  try {
    ...
    if (match) {
      ...
      if (chainIdRegex.test(cosmosBlock)) {
        const updatedCosmosBlock = cosmosBlock.replace(
          chainIdRegex,
          `chainId: ${chainId}`,
        );
        const updatedContent = configContent.replace(
          cosmosBlockRegex,
          `cosmos: {${updatedCosmosBlock}}`,
        );
      ...
      logger.info(
        "Please check if your Hardhat config has a cosmos network configuration",
      );
...
function syncConfiguration() {
  ...
  const hardhatConfigPath = path.join(
    __dirname,
    "./suites/precompiles/hardhat.config.js",
  );
...
async function main() {
  ...
  try {
    ...
    if (configPaths) {
      restoreHardhatConfig(
        configPaths.hardhatConfigPath,
