...
describe("Precompile Revert Cases E2E Tests", function () {
  ...
  before(async function () {
    ...
    const RevertTestContractFactory =
      await hre.ethers.getContractFactory("RevertTestContract");
    ...
    const PrecompileWrapperFactory =
      await hre.ethers.getContractFactory("PrecompileWrapper");
    ...
    invalidValidatorAddress =
      "integravaloper10jmp6sgh4cc6zt3e8gw05wavvejgr5pinvalid";

    console.log(
      "RevertTestContract deployed at:",
      await revertTestContract.getAddress(),
    );
    console.log(
      "PrecompileWrapper deployed at:",
      await precompileWrapper.getAddress(),
    );
  ...
  describe("Direct Precompile Call Reverts", function () {
    it("should handle direct staking precompile revert", async function () {
      try {
        const tx = await revertTestContract.directStakingRevert(
          invalidValidatorAddress,
          { gasLimit: LARGE_GAS_LIMIT },
        );
        await tx.wait();
    ...
    it("should handle direct distribution precompile revert", async function () {
      try {
        const tx = await revertTestContract.directDistributionRevert(
          invalidValidatorAddress,
          { gasLimit: LARGE_GAS_LIMIT },
        );
        await tx.wait();
  ...
  describe("Precompile Call Via Contract Reverts", function () {
    it("should handle precompile call via contract revert", async function () {
      try {
        const tx = await revertTestContract.precompileViaContractRevert(
          invalidValidatorAddress,
          { gasLimit: LARGE_GAS_LIMIT },
        );
        await tx.wait();
    ...
    it("should handle multiple precompile calls with revert", async function () {
      try {
        const tx = await revertTestContract.multiplePrecompileCallsWithRevert(
          invalidValidatorAddress,
          { gasLimit: LARGE_GAS_LIMIT },
        );
        await tx.wait();
    ...
    it("should handle wrapper contract precompile revert", async function () {
      try {
        const tx = await precompileWrapper.wrappedStakingCall(
          invalidValidatorAddress,
          1,
          { gasLimit: LARGE_GAS_LIMIT },
        );
    ...
    it("should capture wrapper revert reason via transaction receipt", async function () {
      try {
        const tx = await precompileWrapper.wrappedDistributionCall(
          invalidValidatorAddress,
          { gasLimit: LARGE_GAS_LIMIT },
        );
        await tx.wait();
  ...
  describe("Precompile OutOfGas Error Cases", function () {
    it("should handle direct precompile OutOfGas", async function () {
      ...
      try {
        const tx = await revertTestContract.directStakingOutOfGas(
          validValidatorAddress,
          { gasLimit: LOW_GAS_LIMIT },
        );
        await tx.wait();
    ...
    it("should handle precompile via contract OutOfGas", async function () {
      try {
        const tx = await revertTestContract.precompileViaContractOutOfGas(
          validValidatorAddress,
          { gasLimit: LOW_GAS_LIMIT },
        );
        await tx.wait();
    ...
    it("should handle wrapper precompile OutOfGas", async function () {
      try {
        const tx = await precompileWrapper.wrappedOutOfGasCall(
          validValidatorAddress,
          { gasLimit: LOW_GAS_LIMIT },
        );
        await tx.wait();
      ...
      const testCases = [
        {
          ...
          call: () =>
            revertTestContract.directStakingRevert(invalidValidatorAddress, {
              gasLimit: LARGE_GAS_LIMIT,
            }),
        ...
        },
        {
          ...
          call: () =>
            revertTestContract.directDistributionRevert(
              invalidValidatorAddress,
              { gasLimit: LARGE_GAS_LIMIT },
            ),
          expectedReason: "invalid validator address",
        ...
        if (error.receipt) {
          ...
          try {
            ...
            await hre.ethers.provider.call({
              to: contractAddress,
              data: revertTestContract.interface.encodeFunctionData(
                "directStakingRevert",
                [invalidValidatorAddress],
              ),
              gasLimit: LARGE_GAS_LIMIT,
