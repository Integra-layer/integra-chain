...
describe("Gov Precompile", function () {
  ...
  const COSMOS_ADDR = "integra1cml96vmptgw99syqrrz8az79xer2pcgp5v6gvh";
  const GOV_MODULE_ADDR = "integra10d07y265gmmuvt4z0w9aw880jnsr700jt6ewul";
  ...
  before(async () => {
    ...
    const tx = await gov
      .connect(signer)
      .submitProposal(signer.address, jsonProposal, [deposit], {
        gasLimit: GAS_LIMIT,
      });
  ...
  it("deposits on the global proposal", async function () {
    ...
    const depTx = await gov
      .connect(signer)
      .deposit(signer.address, globalProposalId, [deposit], {
        gasLimit: GAS_LIMIT,
      });
    const depRcpt = await waitWithTimeout(depTx, 20000, RETRY_DELAY_FUNC);

    // Check balances after deposit
    const signerBalanceAfter = await hre.ethers.provider.getBalance(
      signer.address,
    );
  ...
  it("votes on the global proposal", async function () {
    const voteTx = await gov
      .connect(signer)
      .vote(signer.address, globalProposalId, 1, "simple vote", {
        gasLimit: GAS_LIMIT,
      });
  ...
  it("queries specific deposit for the global proposal", async function () {
    const depositResult = await gov.getDeposit(
      globalProposalId,
      signer.address,
    );
  ...
  it("queries all proposals", async function () {
    ...
    const result = await gov.getProposals(
      0,
      signer.address,
      signer.address,
      pagination,
  ...
  it("cancels a proposal", async function () {
    ...
    const cancelTx = await gov
      .connect(signer)
      .cancelProposal(signer.address, proposalIdToCancel, {
        gasLimit: GAS_LIMIT,
      });
    ...
    expect(signerBalanceAfter).to.equal(
      signerBalanceBefore + expectedRefund - gasFee,
    );
