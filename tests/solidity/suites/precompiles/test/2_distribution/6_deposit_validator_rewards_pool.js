...
describe("Distribution – deposit validator rewards pool", function () {
  ...
  const VAL_BECH32 = "integravaloper10jmp6sgh4cc6zt3e8gw05wavvejgr5pw85ajed";
  ...
  it("deposits rewards and emits DepositValidatorRewardsPool event", async function () {
    ...
    const tx = await distribution
      .connect(signer)
      .depositValidatorRewardsPool(signer.address, VAL_BECH32, [coin], {
        gasLimit: GAS_LIMIT,
      });
    ...
    const expectedBalance =
      balanceBefore - BigInt(coin.amount.toString()) - gasUsed;
    expect(balanceAfter).to.equal(
      expectedBalance,
      "User balance should decrease by deposit amount plus gas costs",
    );
