...
describe("Distribution – withdraw delegator reward", function () {
  ...
  before(async () => {
    ...
    distribution = await hre.ethers.getContractAt(
      "DistributionI",
      DIST_ADDRESS,
    );
  });
  ...
  it("should withdraw rewards and emit WithdrawDelegatorReward event", async function () {
    ...
    const balanceBefore =
      await hre.ethers.provider.getBalance(newWithdrawAddress);
    console.log("User balance before withdrawal:", balanceBefore.toString());

    const tx = await distribution
      .connect(signer)
      .withdrawDelegatorRewards(signer.address, valBech32, {
        gasLimit: GAS_LIMIT,
      });
    ...
    expect(evt.args.amount).to.be.greaterThan(
    ...
    expect(balanceAfter).to.be.gte(
      expectedMinBalance,
      "User balance should increase by withdrawn rewards minus gas costs",
    );
    console.log("finished balance checks");

    // Check state after withdrawal
    const afterResult = await distribution.delegationRewards(
      signer.address,
      valBech32,
    );
    if (afterResult.length > 0) {
      ...
      expect(afterReward.amount).to.be.lessThan(
        currentReward.amount,
