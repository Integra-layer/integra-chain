...
describe("Distribution – fund community pool", function () {
  ...
  it("funds the community pool and emits FundCommunityPool event", async function () {
    const coin = { denom: "airl", amount: hre.ethers.parseEther("0.01") };
    ...
    expect(balanceAfter).to.equal(
      expectedBalance,
      "User balance should decrease by funding amount plus gas costs",
    );
    ...
    expect(end).to.gte(
      start + coin.amount,
      "Community pool should increase by funded amount",
    );
  });
});
