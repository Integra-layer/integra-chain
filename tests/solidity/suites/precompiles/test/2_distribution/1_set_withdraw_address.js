...
describe("Distribution – set withdraw address", function () {
  ...
  it("should set withdraw address and emit SetWithdrawerAddress event", async function () {
    const newWithdrawAddress = "integra1fx944mzagwdhx0wz7k9tfztc8g3lkfk6s6nf2m";
    const tx = await distribution
      .connect(signer)
      .setWithdrawAddress(signer.address, newWithdrawAddress, {
        gasLimit: GAS_LIMIT,
      });
    ...
    const withdrawer = await distribution.delegatorWithdrawAddress(
      signer.address,
    );
