...
describe("Staking – edge case revert test", function () {
  ...
  before(async function () {
    ...
    const StakingReverterFactory =
      await hre.ethers.getContractFactory("StakingReverter");
    stakingReverter = await StakingReverterFactory.deploy({
      value: hre.ethers.parseEther("1.0"), // Fund contract with 1 ETH
      gasLimit: GAS_LIMIT,
    });
    await waitWithTimeout(
      stakingReverter.deploymentTransaction(),
      20000,
      RETRY_DELAY_FUNC,
    );

    validatorAddress = "integravaloper10jmp6sgh4cc6zt3e8gw05wavvejgr5pw85ajed";

    console.log(
      "StakingReverter deployed at:",
  ...
  describe("Edge case: callPrecompileBeforeAndAfterRevert with numTimes=1", function () {
    it("should execute exactly two delegate operations", async function () {
      ...
      [initialShares, initialBalance] = await staking.delegation(
        contractAddress,
        validatorAddress,
      );
      console.log("Initial delegation shares:", initialShares.toString());
      console.log(
        "Initial delegation balance:",
      ...
      const tx = await stakingReverter.callPrecompileBeforeAndAfterRevert(
        1,
        validatorAddress,
        {
          gasLimit: GAS_LIMIT,
      ...
      const [finalShares, finalBalance] = await staking.delegation(
        contractAddress,
        validatorAddress,
      );
      ...
      console.log(
        "✓ Edge case test passed: exactly two delegate operations executed",
      );
    });
  });
});
