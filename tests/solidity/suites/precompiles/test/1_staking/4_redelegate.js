...
function formatRedelegation(res) {
  ...
  const entries = rawEntries.map((entry) => {
    const [creationHeight, completionTime, initialBalance, sharesDst] = entry;
...
describe("Staking – redelegate with event and state assertions", function () {
  ...
  it("should redelegate tokens and emit Redelegate event", async function () {
    const signerBech32 = "integra1cml96vmptgw99syqrrz8az79xer2pcgp5v6gvh";
    const srcValBech32 =
      "integravaloper10jmp6sgh4cc6zt3e8gw05wavvejgr5pw85ajed";
    const dstValBech32 =
      "integravaloper1cml96vmptgw99syqrrz8az79xer2pcgpjja8n6";
    ...
    const beforeSrcDelegationRaw = await staking.delegation(
      signer.address,
      srcValBech32,
    );
    const beforeDstDelegationRaw = await staking.delegation(
      signer.address,
      dstValBech32,
    );
    ...
    console.log(
      "Before redelegation - srcVal delegation shares:",
      beforeSrcDelegation.shares.toString(),
    );
    console.log(
      "Before redelegation - srcVal delegation balance:",
      beforeSrcDelegation.balance.amount.toString(),
      beforeSrcDelegation.balance.denom,
    );
    console.log(
      "Before redelegation - dstVal delegation shares:",
      beforeDstDelegation.shares.toString(),
    );
    console.log(
      "Before redelegation - dstVal delegation balance:",
      beforeDstDelegation.balance.amount.toString(),
      beforeDstDelegation.balance.denom,
    );

    // 2) query redelegation entries before
    const beforeRaw = await staking.redelegation(
      signer.address,
      srcValBech32,
      dstValBech32,
    );
    ...
    const tx = await staking
      .connect(signer)
      .redelegate(signer.address, srcValBech32, dstValBech32, amount, {
        gasLimit: GAS_LIMIT,
      });
    ...
    const afterSrcDelegationRaw = await staking.delegation(
      signer.address,
      srcValBech32,
    );
    const afterDstDelegationRaw = await staking.delegation(
      signer.address,
      dstValBech32,
    );
    ...
    console.log(
      "After redelegation - srcVal delegation shares:",
      afterSrcDelegation.shares.toString(),
    );
    console.log(
      "After redelegation - srcVal delegation balance:",
      afterSrcDelegation.balance.amount.toString(),
      afterSrcDelegation.balance.denom,
    );
    console.log(
      "After redelegation - dstVal delegation shares:",
      afterDstDelegation.shares.toString(),
    );
    console.log(
      "After redelegation - dstVal delegation balance:",
      afterDstDelegation.balance.amount.toString(),
      afterDstDelegation.balance.denom,
    );
    ...
    const expectedSrcShares =
      beforeSrcDelegation.balance.amount === amount
        ? 0n
        : beforeSrcDelegation.shares - amountWithPrecision;
    ...
    expect(afterSrcDelegation.balance.denom).to.equal(
      beforeSrcDelegation.balance.denom,
    );
    expect(afterDstDelegation.balance.denom).to.equal(
      beforeDstDelegation.balance.denom,
    );

    // 6) query redelegation state after
    const afterRaw = await staking.redelegation(
      signer.address,
      srcValBech32,
      dstValBech32,
    );
    ...
    expect(responses.length).to.be.gte(
      1,
      "redelegations() should return at least one response",
    );
    ...
    expect(redelegation.validatorSrcAddress).to.equal(
      afterR.validatorSrcAddress,
    );
    expect(redelegation.validatorDstAddress).to.equal(
      afterR.validatorDstAddress,
    );
    ...
    expect(BigInt(entryResp.balance.toString())).to.equal(
      afterR.entries[0].initialBalance,
