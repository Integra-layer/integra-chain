...
describe("Staking – delegate, undelegate & cancelUnbondingDelegation with event assertions", function () {
  ...
  it("should delegate, undelegate, then cancel unbonding and emit correct events", async function () {
    const valBech32 = "integravaloper10jmp6sgh4cc6zt3e8gw05wavvejgr5pw85ajed";
    const amount = hre.ethers.parseEther("0.001");

    // DELEGATE
    const delegateTx = await staking
      .connect(signer)
      .delegate(signer.address, valBech32, amount, { gasLimit: GAS_LIMIT });
    const delegateReceipt = await waitWithTimeout(
      delegateTx,
      20000,
      RETRY_DELAY_FUNC,
    );
    console.log(
      ...
      delegateTx.hash,
      "gas used:",
      delegateReceipt.gasUsed.toString(),
    );

    const hexValAddr = "0x7cB61D4117AE31a12E393a1Cfa3BaC666481D02E";
    const delegateEvt = findEvent(
      delegateReceipt.logs,
      staking.interface,
      "Delegate",
    );
    ...
    const beforeRaw = await staking.unbondingDelegation(
      signer.address,
      valBech32,
    );
    ...
    // UNDELEGATE
    const undelegateTx = await staking
      .connect(signer)
      .undelegate(signer.address, valBech32, amount, { gasLimit: GAS_LIMIT });
    const undelegateReceipt = await waitWithTimeout(
      undelegateTx,
      20000,
      RETRY_DELAY_FUNC,
    );
    console.log(
      ...
      undelegateTx.hash,
      "gas used:",
      undelegateReceipt.gasUsed.toString(),
    );

    const unbondEvt = findEvent(
      undelegateReceipt.logs,
      staking.interface,
      "Unbond",
    );
    ...
    const afterRaw = await staking.unbondingDelegation(
      signer.address,
      valBech32,
    );
    ...
    const cancelReceipt = await waitWithTimeout(
      cancelTx,
      20000,
      RETRY_DELAY_FUNC,
    );
    console.log(
      "CancelUnbondingDelegation tx hash:",
      cancelTx.hash,
      "gas used:",
      cancelReceipt.gasUsed.toString(),
    );

    const cancelEvt = findEvent(
      cancelReceipt.logs,
      staking.interface,
      "CancelUnbondingDelegation",
    );
    expect(cancelEvt, "CancelUnbondingDelegation event should be emitted").to
      .exist;
    ...
    expect(cancelEvt.args.creationHeight).to.equal(
      entryToCancel.creationHeight,
    );

    // VERIFY ENTRY REMOVAL
    const finalRaw = await staking.unbondingDelegation(
      signer.address,
      valBech32,
    );
