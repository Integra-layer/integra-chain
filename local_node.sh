...
CHAINID="${CHAIN_ID:-integra_26217-1}"
...
LOGLEVEL="info"
# Set dedicated home directory for the intgd instance
CHAINDIR="$HOME/.intgd"
...
Options:
  ...
  --mnemonic-file PATH     Where to write mnemonics YAML (default: \$HOME/.intgd/mnemonics.yaml)
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -n)
      ...
      ;;
    --no-install)
      echo "Flag --no-install passed -> Skipping installation of the intgd binary."
      install=false; shift
      ;;
    --remote-debugging)
...
add_genesis_funds() {
  local keyname="$1"
  intgd genesis add-genesis-account "$keyname" 1000000000000000000000airl --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  intgd config set client chain-id "$CHAINID" --home "$CHAINDIR"
  intgd config set client keyring-backend "$KEYRING" --home "$CHAINDIR"

  # ---------------- Validator key ----------------
  VAL_KEY="mykey"
  VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"
  echo "$VAL_MNEMONIC" | intgd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  echo "$VAL_MNEMONIC" | intgd init $MONIKER -o --chain-id "$CHAINID" --home "$CHAINDIR" --recover

  # ---------- Genesis customizations ----------
  jq '.app_state["staking"]["params"]["bond_denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["expedited_min_deposit"][0]["denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["evm"]["params"]["evm_denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["mint"]["params"]["mint_denom"]="airl"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for Integra Layer.","denom_units":[{"denom":"airl","exponent":0,"aliases":["attoirl"]},{"denom":"irl","exponent":18,"aliases":[]}],"base":"airl","display":"irl","name":"IRL","symbol":"IRL","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"airl",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  intgd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000airl --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  for ((i=0; i<${#dev_mnemonics[@]}; i++)); do
    ...
    echo "$mnemonic" | intgd keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  if [[ "$ADDITIONAL_USERS" -gt 0 ]]; then
    ...
    for ((i=0; i<ADDITIONAL_USERS; i++)); do
      ...
      mnemonic_out="$(intgd keys add "$keyname" --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR" 2>&1)"
  ...
  # --------- Finalize genesis ---------
  intgd genesis gentx "$VAL_KEY" 1000000000000000000000airl --gas-prices ${BASEFEE}airl --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
  intgd genesis collect-gentxs --home "$CHAINDIR"
  intgd genesis validate-genesis --home "$CHAINDIR"
