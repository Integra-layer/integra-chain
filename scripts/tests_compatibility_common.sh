...
# Start intgd node in background
start_node() {
	...
	if [ "$print_log" = true ]; then
		...
		./local_node.sh -y >/tmp/intgd.log 2>&1 &
	fi
	NODE_PID=$!
	popd >/dev/null
}
...
wait_for_node() {
	...
	local timeout=60
	local elapsed=0
	echo "Waiting for intgd node to be ready..."
	...
	if [ $elapsed -ge $timeout ]; then
		...
		tail -20 /tmp/intgd.log 2>/dev/null || echo "No intgd logs found"
		exit 1
	fi
}
